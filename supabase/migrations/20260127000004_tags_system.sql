  ...
  WITH matched_patterns AS (
    ...
    WHERE tp.organization_id = p_org_id
      ...
      AND (
        ...
        OR (p_amount * 100)::BIGINT <@ tp.amount_range
      )
...
BEGIN
  -- 获取交易信息
  SELECT vendor_name, (total_amount * 100)::BIGINT, organization_id
  ...
  -- 更新 ML 模式（如果用户确认了）
  IF p_user_confirmed AND v_vendor_name IS NOT NULL THEN
    ...
    DO UPDATE SET
      suggested_tags = CASE 
        WHEN p_tag_id = ANY(COALESCE(tag_patterns.suggested_tags, ARRAY[]::UUID[])) 
        THEN tag_patterns.suggested_tags
        ELSE array_append(
...
BEGIN
  IF p_match_all THEN
    ...
    GROUP BY t.id, t.vendor_name, t.total_amount, t.transaction_date
...
-- 注意：系统标签通过 create_default_tags() 函数为每个组织创建
-- 这里不直接插入，而是通过函数调用
...
BEGIN
  ...
  ) AS v(org_id, tag_name, display, tag_color, tag_category, is_system)
  WHERE NOT EXISTS (
    SELECT 1 FROM tags 
    WHERE organization_id = v.org_id 
    AND name = v.tag_name
  );
