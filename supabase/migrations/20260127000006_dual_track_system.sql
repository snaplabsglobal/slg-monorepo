...
BEGIN
  -- 获取收据信息
  SELECT vendor_name, organization_id
...
BEGIN
  ...
  WITH tag_totals AS (
    SELECT 
      t.name as tag_name,
      SUM((tr.total_amount * 100)::BIGINT) as total_cents
...
AS $$
  SELECT 
    ac.code,
    ac.name_en,
    SUM((t.total_amount * 100)::BIGINT) as total_amount_cents,
    '$' || SUM(t.total_amount)::TEXT as total_amount_display,
    SUM(
      CASE 
        WHEN ac.is_tax_deductible THEN 
          (t.total_amount * 100)::BIGINT * COALESCE(ac.tax_deduction_rate, 1.0)
        ELSE 0
      END
    ...
    '$' || SUM(
      CASE 
        WHEN ac.is_tax_deductible THEN 
          t.total_amount * COALESCE(ac.tax_deduction_rate, 1.0)
        ELSE 0
      END
    )::TEXT as deductible_amount_display,
