...
CREATE TABLE IF NOT EXISTS transaction_splits (
  ...
  -- 拆分状态
  split_status TEXT NOT NULL DEFAULT 'active' CHECK (split_status IN (
    'active',       -- 生效中
    'cancelled',    -- 已取消
    'superseded'    -- 被新拆分替代
  )),
  ...
  -- 验证字段
  total_split_amount_cents BIGINT NOT NULL,  -- 拆分总金额（分）
  validation_passed BOOLEAN DEFAULT false,    -- 是否通过验证
...
BEGIN
  -- 获取原始收据金额
  SELECT amount_cents INTO v_original_amount
  FROM transactions
