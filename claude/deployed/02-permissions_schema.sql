...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  subscription_tier TEXT NOT NULL DEFAULT 'free' CHECK (subscription_tier IN (
    'free',           -- 未付费用户
    'basic_ls',       -- 仅 LedgerSnap
    'basic_jss',      -- 仅 JobSite Snap
    'pro_ls',         -- LedgerSnap Pro
    'pro_jss',        -- JobSite Snap Pro
    'enterprise'      -- 全功能
  )),
  
  -- 可访问的应用列表（数组）
  accessible_apps TEXT[] DEFAULT ARRAY['ledgersnap']::TEXT[],
...
-- 用户权限查询优化
CREATE INDEX idx_profiles_accessible_apps ON profiles USING GIN (accessible_apps);
CREATE INDEX idx_profiles_subscription_tier ON profiles(subscription_tier);
CREATE INDEX idx_profiles_email ON profiles(email);

-- 访问日志查询优化
CREATE INDEX idx_access_logs_user_app ON app_access_logs(user_id, app_code, created_at DESC);
CREATE INDEX idx_access_logs_created ON app_access_logs(created_at DESC);

-- 升级请求查询优化
CREATE INDEX idx_upgrade_requests_user ON upgrade_requests(user_id, request_status);
CREATE INDEX idx_upgrade_requests_status ON upgrade_requests(request_status, created_at DESC);
