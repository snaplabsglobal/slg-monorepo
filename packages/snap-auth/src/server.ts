 ...
 * 
 * For SSO: All apps must use the same Supabase project and cookie domain
 */
export async function createClient() {
  const cookieStore = await cookies()

  // NOTE: Our `Database` typing is intentionally lightweight and may not satisfy
  // Supabase's strict generic constraints in all consuming apps. Use `any` here
  // to avoid `never` inference breaking builds.
