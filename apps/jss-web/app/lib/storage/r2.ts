...
// Local R2 storage utilities (replaces @slo/snap-storage/server for Vercel build)
...
export function getR2Config(): R2Config {
  ...
  if (!accountId || !accessKeyId || !secretAccessKey || !bucketName) {
    throw new Error(
      'Cloudflare R2 credentials not configured. Required: CLOUDFLARE_ACCOUNT_ID (or R2_ENDPOINT), CLOUDFLARE_R2_ACCESS_KEY_ID (or R2_ACCESS_KEY_ID), CLOUDFLARE_R2_SECRET_ACCESS_KEY (or R2_SECRET_ACCESS_KEY), CLOUDFLARE_R2_BUCKET_NAME (or R2_BUCKET_NAME)'
    )
  }
